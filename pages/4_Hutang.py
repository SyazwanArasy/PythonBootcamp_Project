...
.status-selesai { color: #4caf50; font-size: 12px; font-weight: 600; text-transform: uppercase; margin-top: 3px; margin-bottom: 12px; }
</style>
...
with col1:
    ...
    <div class="hutang-total-red hutang-total">
        <div class="label">{t("saya_hutang")}</div>
...
with col2:
    ...
    <div class="hutang-total-green hutang-total">
        <div class="label">{t("orang_hutang")}</div>
...
sections = [
    (t("saya_hutang"), "saya_hutang"),
    (t("orang_hutang"), "orang_hutang"),
]
...
for col, (label, direction) in zip(list_cols, sections):
    with col:
        title_col, addbtn_col = st.columns([3.4, 0.8])
        with title_col:
            st.subheader(label)
        with addbtn_col:
            with st.popover("➕ Add hutang"):
